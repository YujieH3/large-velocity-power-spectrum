...
int main(int argc, char **argv)
{
	...
	dataPts = annAllocPts(maxPts, dim);			// allocate data points // seems the only place maxPts is used --Jay
	nnIdx = new ANNidx[k];						// allocate near neigh indices
	dists = new ANNdist[k];						// allocate near neighbor dists

	nPts = 0;									// read data points

	// no need to print data points --Jay
	// cout << "Data Points:\n";
	...
	while (readPt(*queryIn, queryPt)) {			// read query points
		// cout << "Query point: ";				// echo query point // remove this line, easier to read
