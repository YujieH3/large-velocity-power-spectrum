...
from velocity_power.folding import SimulationField3D, SimulationParticles, _vec_to_vm_grid, BlocksDecomposition

def voxelize_interp_to_field(self, Nsize, smoothing_rate=1., auto_padding=True):
...
def voxelize_interp_to_blocks(self, run_output_dir, nblocks, Nblock, smoothing_rate=1.):
  """
  ...
  for r in range(nblocks): # 0, 1, 2, ..., Nblock-1
    for s in range(nblocks):  # Nblock*Lbox_blk=Lbox
      for t in range(nblocks):
        ...
        subField = blockParticles.voxelize_interp_to_field(Nsize=Nblock, 
